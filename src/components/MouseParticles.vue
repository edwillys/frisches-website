...
function createParticle(): Particle {
  ...
  for (let i = 0; i < CONSTANTS.SHAPE_SEGMENTS; i++) {
    shapeOffsets.push(
      1 - CONSTANTS.SHAPE_IRREGULARITY + Math.random() * CONSTANTS.SHAPE_IRREGULARITY * 2
    )
  }
  ...
  return {
    ...
    logoOrbitRadius:
      CONSTANTS.LOGO_ORBIT_RADIUS + (Math.random() - 0.5) * CONSTANTS.LOGO_ORBIT_VARIANCE,
    logoTransition: 0,
  }
}
  ...
  for (const p of particles) {
    if (p.isLogoAttracted) {
      ...
      if (mouseMoving && dist < CONSTANTS.MOUSE_ATTRACTION_RADIUS) {
        const force =
          (1 - dist / CONSTANTS.MOUSE_ATTRACTION_RADIUS) * CONSTANTS.MOUSE_ATTRACTION_STRENGTH
        p.vx += dx * force
        p.vy += dy * force
        p.isAttracted = true
        p.attractionTime = 60
...
function drawParticles() {
  ...
  for (const p of particles) {
    ...
    const alpha =
      CONSTANTS.GLOW_MIN_ALPHA +
      glowIntensity * (CONSTANTS.GLOW_MAX_ALPHA - CONSTANTS.GLOW_MIN_ALPHA)
    const blur =
      CONSTANTS.GLOW_BLUR_MIN + glowIntensity * (CONSTANTS.GLOW_BLUR_MAX - CONSTANTS.GLOW_BLUR_MIN)
