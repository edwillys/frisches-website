...
4. **Build core home screen components** - Create `views/HomeView.vue` as main container, implement `components/CardDealer.vue` with `Intro_Voyante_00020.jpg` background, build `components/MenuCard.vue` for interactive menu items (Music, About, Tour) using `Voyante_Carte_1/2/3_00000.jpg` images
